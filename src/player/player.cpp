...
void Player::Update(float dt) {
  ...
  if (GameLib::CollisionDetector::collision(dribble_circle,
                                            ball->GetCollidable())) {
...
void Player::animate() {
  ...
  player_sprite->shadow->SetPosition(
      player_sprite->GetPosition().x +
          3, /* TODO magic numbers here for shadow offset */
...
void Player::do_dribble(const GameLib::Vector3 &direction) {
  // TODO height
  if (ball->physical->position.z > 30) return;
...
void Player::do_close_control() {
  ...
  t = t + last_direction.getNormalizedToUnits() * (close_control_circle.radius);
